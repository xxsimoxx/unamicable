	...
	<section id="comments-area" class="comments-area">
		<?php if ( have_comments() ) { ?>
			<h1 class="comments-title">
				...
				<?php
				if ( '1' === $count ) {
					printf( esc_html_x( 'One Comment', 'comments title', 'silver-quantum' ) );
				} else {
					// Translators: 1 = counts.
					printf( _nx( '%1$s Comment', '%1$s Comments', absint( $count ), 'comments title', 'silver-quantum' ), absint( number_format_i18n( $count ) ) ); // phpcs:ignore
				}
				?>
			</h1>
		<?php } ?>
